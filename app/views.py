...
def compose():
  ...
  if form.validate_on_submit():
    if requests.post(url_for('new_post', handle=u['username']), json={'object': form.text.data}, headers=sign_headers(u, API_CONTENT_HEADERS)):
...
def viewPost(handle, postID):
  p = mongo.db.posts.find_one({'object.id': 'https://'+API_NAME+'/'+handle+'/posts/'+postID})
  return str(p)
